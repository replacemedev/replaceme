      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 md:gap-8 items-stretch">
        {orderedPlans.map((plan) => {
          ...
          return (
            <div
              key={plan.id}
              className={`relative flex flex-col justify-between p-6 md:p-8 rounded-3xl transition-all duration-300 ${isGrowth
                  ...
                  {detail.features.map((feature, idx) => (
                    <li key={idx} className="flex items-start gap-2.5">
                      {feature.included ? (
                        ...
                        <span
                          className="mt-[4px] inline-flex size-4 shrink-0 items-center justify-center rounded-full bg-red-50"
                          aria-hidden
                        >
                      ...
                      <span
                        className={`min-w-0 text-xs font-medium leading-snug ${feature.included ? "text-gray-700" : "text-gray-400 line-through"
                          }`}
                      >
                        {feature.text}
                      </span>
              ...
              <div className="mt-auto pt-8 space-y-2">
                {isCurrent ? (
                  ...
                  <button
                    ...
                    className={`w-full py-3.5 px-4 rounded-xl font-bold text-sm transition-all duration-200 cursor-pointer flex items-center justify-center text-center ${isGrowth
