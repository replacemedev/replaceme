...
export async function updateSession(request: NextRequest) {
  ...
  try {
    ...
    if (!env) {
      // Allow public routes (including /signin) to render; auth checks will fail closed elsewhere.
    ...
    if (user) {
      ...
      if (role === "employer" && (isWorkerRoute || isAdminRoute)) {
        return NextResponse.redirect(
          new URL(ROLE_HOME_PATH.employer, request.url)
        );
      }
    ...
    console.error(
      "[auth/middleware] Session refresh failed; allowing request through.",
      error
    );
