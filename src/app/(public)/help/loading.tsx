    ...
    <main className={`bg-slate-50/50 min-h-[calc(100vh-4rem)] pb-16 ${PUBLIC_PAGE_TOP}`}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center max-w-3xl mx-auto mb-12 sm:mb-16 flex flex-col items-center">
          ...
          <SkeletonBlock className="h-10 sm:h-12 w-3/4 sm:w-2/3 rounded-xl bg-slate-200 mb-4 animate-pulse" />
            ...
            <section key={cat.id} className="space-y-6">
              ...
              <div className="flex items-center justify-between pb-4 border-b border-slate-200/70">
                <div className="flex items-center gap-3">
                  <SkeletonBlock className="w-10 h-10 rounded-xl bg-slate-200 shrink-0 animate-pulse" />
                ...
                <SkeletonBlock className="h-4 w-16 rounded-md bg-slate-200 animate-pulse" />
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-8">
                {Array.from({ length: cat.cardCount }).map((_, index) => (
                  <div
                    key={index}
                    className="bg-white rounded-2xl p-6 border border-slate-200/80 shadow-xs flex flex-col justify-between h-full animate-pulse"
                  >
                    <div>
                      {/* Icon Placeholder */}
                      <SkeletonBlock className="w-12 h-12 rounded-xl bg-slate-200 mb-4 shrink-0" />
                      {/* Title Placeholder */}
